...
def load_network():
    ...
    iter = ckpt['iter'] + 1
    print('load network from ', ckpt_path + f' at iter {iter}')
    return model.cuda().deploy_mlps_to_secondary_gpus(), iter
...
def _freeview(
    ...
    model, iter = load_network()
    test_loader = create_dataloader(data_type)
    model.load_smpl(test_loader.dataset.avg_beta)
    writer = ImageWriter(
...
def run_movement(render_folder_name='movement'):
    ...
    for idx, batch in enumerate(tqdm(test_loader)):
        ...
        with torch.no_grad():
            net_output = model(**data, iter_val=iter)
        ...
        writer.append(img_out, img_name=f"{int(frame_name[-4:]):06d}")
