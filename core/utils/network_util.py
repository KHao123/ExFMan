...
class ConvDecoder2D(nn.Module):
    r""" Convolutional 2D volume decoder."""
    ...
    def __init__(self, embedding_size=256, area_size=128):
        ...
        self.block_mlp = nn.Sequential(nn.Linear(embedding_size, 1024), 
                                       nn.LeakyReLU(0.2))
        block_conv = []
        inchannels, outchannels = 256, 128
        for _ in range(int(np.log2(area_size)) - 2):
            block_conv.append(nn.ConvTranspose2d(inchannels, 
                                                 outchannels, 
                                                 4, 2, 1))
            block_conv.append(nn.LeakyReLU(0.2))
            if inchannels == outchannels:
    ...
    def forward(self, embedding):
        """ 
            Args:
                embedding: Tensor (B, N)
        """    
        return self.block_conv(self.block_mlp(embedding).view(-1, 256, 2, 2))
